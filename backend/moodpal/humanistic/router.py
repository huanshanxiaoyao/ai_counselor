...
class HumanisticTechniqueRouter(TechniqueRouter[HumanisticGraphState]):
    def __init__(self, registry: Optional[HumanisticNodeRegistry] = None):
        self.registry = registry or HumanisticNodeRegistry()

    def route(self, state: HumanisticGraphState) -> TechniqueSelection:
        ...
        if router_config.HOLDING_ROUTE_RULE.predicate(state):
            return self._build_selection_from_rule(router_config.HOLDING_ROUTE_RULE)

        fallback_selection = self._route_circuit_breaker_fallback(state)
        if fallback_selection:
            return fallback_selection

        if router_config.ACCEPTING_ROUTE_RULE.predicate(state):
            return self._build_selection_from_rule(router_config.ACCEPTING_ROUTE_RULE)

        if router_config.BODY_FOCUS_ROUTE_RULE.predicate(state):
            return self._build_selection_from_rule(router_config.BODY_FOCUS_ROUTE_RULE)

        return self._build_selection_from_rule(router_config.CLARIFYING_ROUTE_RULE)

    def get_same_phase_fallbacks(self, technique_id: str) -> tuple[str, ...]:
        return router_config.SAME_PHASE_FALLBACKS.get(technique_id, ())

    def _route_circuit_breaker_fallback(self, state: HumanisticGraphState) -> Optional[TechniqueSelection]:
        ...
        if fallback_action == 'regress_to_holding':
            return self._build_selection_from_rule(router_config.HOLDING_ROUTE_RULE)
